...
function setup() {
	...
	init_noise_row_pos =  p.width/2-10;
}
...
function draw() {
	...
	for(var j=noise_buffer.length-1; j>=0; j--){
		...
		if(new_pos > p.height){
			...
			p.line(noise_buffer[j][0]+1, noise_buffer[j][1], noise_buffer[j][0]+noise_buffer[j][2], noise_buffer[j][1]);
		}
	}
	...
	if(regenerate){
		...
		for(var k=0;k<side_buffer.length;k++){
			...
			p.line(0,side_buffer[k][0], side_buffer[k][1], side_buffer[k][0]);
		}
	}
	...
	if(noise_row_toggle>4){
		...
		for(var i=0;i<noise_row_buffer.length;i++){
			...
			if(distort_lines){
				...
				p.line(line_x-distort2,p.height/2-15+i+distort1, line_x+20+distort2, p.height/2-15+i+distort2);
			}else{
				p.line(line_x,p.height/2-15+i, line_x+20, p.height/2-15+i);
			}
		}
