...
function draw() {
	...
	for (let row = 0; row<8; row++){
		let sin2 = p.cos(curr_angle2*p.PI/180);
		...
		for(var col=0;col<16; col++){
			...
			// for(var block=0; block<8; block++){
			// 	let sin1 = p.sin(curr_angle);
			// 	curr_angle += (get_inc());
			// 	if(curr_angle>p.TWO_PI){
			// 		curr_angle = 0;
			// 	}
			//
			...
			for(var block=0;block<8; block++){
				...
				if(block == 0){
					sprite_idx = p.int((1+sin1)*64)//p.int(p.random(0,128));
					if(sprite_idx>127){
						sprite_idx=127;
					}
				}
				// if(p.abs(sin1)>sinlimit && sin1>0) send(0x0 | main_sprite[sprite_idx*8+block] )
				//if(p.abs(sin1)>sinlimit && sin1>0)

send( 1<<(p.int(sin1*3)+4) )
continue;
				if(p.abs(sin1)>sinlimit && sin1>0)
				{
					//send( 1<<(p.int(sin1*sin2*3)+4) )

					send(0x0 | main_sprite[sprite_idx*8+block] & (sin1*sin2*127)+(counter) )
				}
				else if(p.abs(sin1)>sinlimit && sin1<0){
					//send(0x00 )
					send( 1<<(p.int(sin1*sin2*127)+counter) )
//					send(0x0 | main_sprite[sprite_idx*8+block] & (sin1*sin2*127)+(127) )
 ...
 //console.log(1>>Math.round(sin1*7))
				...
				// if(p.abs(sin1)>sinlimit && sin1>0) send(0x0 | main_sprite[sprite_idx*8+block]& sin1*(255) )
			//	if(p.abs(sin1)>sinlimit && sin1<0) send(0x0 | main_sprite[sprite_idx*8+block]| p.abs(sin1)*(255))
			//	if(p.abs(sin1)<sinlimit ) send(0x00 )
			}
		}
	}
	counter++;
