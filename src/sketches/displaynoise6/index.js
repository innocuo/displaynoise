...
function draw() {
  ...
  curr_inc = (360 * (p.ceil(300 * vol) - 1)) / 128; // 2000* vol;
  if (curr_inc > 255) {
    curr_inc = 255;
  }
  curr_inc *= -1;
  ...
  for (let row = 0; row < row_total; row += row_height) {
    ...
    for (let col = 0; col < 16; col++) {
      ...
      for (let block = 0; block < 8; block++) {
        ...
        // tmp_angle += get_inc();
        let sin1 = p.sin((tmp_angle * p.PI) / 180);
        ...
        const pixel_pos = p.int((wave_val * (row_total * row_height - 1)) / 2);
        ...
        for (let ii = 0; ii < row_height; ii++) {
          ...
          if (ii == p.int(pixel_pos / 8)) {
            if (pix_val == 0) {
              if (pixel2 > 0) {
                display.send(p.round(curr_inc * -1) >> pixel2);
              }
              // send( pixel2);
            } else {
              display.send(pix_val & p.int((wave_val * 255) / 2));
            }
          } else {
            // send( 0 );
          }
