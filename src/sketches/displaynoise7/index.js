...
display = createDisplay(p);

function setup() {
  p.frameRate(45);
  c = p.createCanvas(display_width, display_height);
  let ac = p.getAudioContext();
...
function draw() {
  ...
  let draw_wave_height = p.ceil(display_rows / wave_count); //unit is # of rows
  let draw_wave_height_in_px = draw_wave_height * 8;
  p.fill(screenColor.r, screenColor.g, screenColor.b);
  p.stroke(screenColor.r, screenColor.g, screenColor.b);

  let vol = amp.getLevel(); //value between 0.0 to 1.0

  if(!angle_increment_fixed){
    angle_increment = ( 1+p.floor(vol * 7719) ) % 360; // value between 1 to 360
  }
  
  let amplitude = 39 * vol; //value between 0 to 20, more volume, more amplitude;
  let y_amplitude_offset = p.round(draw_wave_height_in_px  *(1 - amplitude) * 0.5)
  ...
  for (let row = 0; row < display_rows; row++) {
    ...
    const current_wave = p.ceil((row + 1) / draw_wave_height) - 1;
    ...
    for (let col = 0; col < display_cols; col++) {
      ...
      for (let sub_col = 0; sub_col < 8; sub_col++) {
        ...
        //only increment the angle when we've reached the segment p.width
        if(x_pos % segment_width == 0)
          current_angle += angle_increment;

        let wave_value = p.sin((current_angle * p.PI) / 180) + 1; // value from 0.0 to 2.0
        wave_value *= 0.5 //value is now between 0 to 1.0
        wave_value *= amplitude;
        
        // initial y position
        // + half of empty space of row p.height - amplitude, so it's centered
        // + rounded pixel position between 0 to row p.height in pixels
        const pixel_pos =
          y_offset + 
          y_amplitude_offset+
          p.round(wave_value * (draw_wave_height_in_px -1) ); //say row is 64px, values should go from 0 to 63, that's  why we do a -1
